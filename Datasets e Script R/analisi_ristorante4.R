...
par(mfrow=c(1,1))
plot(definitivo$newdate, definitivo$Vendite_5, ylab = 'Vendite', xlab = 'data', type = 'c', main = 'Ristorante5')
plot(definitivo$newdate, definitivo$Scontrino_medio_5, ylab = 'Scontrino medio', xlab = 'data', type = 'c', main = 'Ristorante5') #da mettere in Appendice

#### SARIMAX VENDITE RISTORANTE 1 ####

covariate <- data.matrix(definitivo[,c(24,25,26,27,28,29,31,35,36)]) 
tsdata5 <- ts(definitivo$Vendite_5, frequency = 7 ) 

# Stagionalità e stazionarietà serie storica (grafici e test):
par(mfrow=c(2,1))
acf(definitivo$Vendite_5)
pacf(definitivo$Vendite_5)
par(mfrow=c(1,1))
lag.plot(definitivo$Vendite_5, lags = 16)
adf.test(definitivo$Vendite_5)

# Stimiamo il miglior modello SARIMAX utilizzando la funzione auto.arima()
autoarima1.1 <- auto.arima(tsdata5,
                           xreg = covariate,
                           trace = FALSE,
                           seasonal = TRUE,
                           stepwise = FALSE,
...
summary(data$Vendite_5)
summary(data$Scontrini_5)
summary(data$Scontrino_medio_5)

# Imputiamo con 0 i valori mancanti (perchè ristoranti chiusi per lockdown)
data$Scontrini_5 <- ifelse(is.na(data$Scontrini_5), 0, data$Scontrini_5)
str(data)

ggplot() + geom_rect(data=data, (aes(xmin=newdate-0.5, xmax=newdate+0.5, 
                                     ymin=min(Scontrini_5)-0.5, ymax=max(Scontrini_5)+0.5, fill=factor(ZONA)))) +
  scale_fill_manual(values=c("orange","yellow", "grey","red"), guide=FALSE) +
  geom_line(data = data, aes(x = newdate, y = Scontrini_5), size=0.8) +
  xlab('Data')+
  ylab('Scontrini')
...
ggplot() + geom_rect(data=data1, (aes(xmin=newdate-0.5, xmax=newdate+0.5, 
  ...
  geom_line(data = data1, aes(x = newdate, y = Scontrini_5), size=0.8) +
  xlab('Data')+
  ylab('Scontrini')
